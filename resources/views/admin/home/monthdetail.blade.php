	...
	<div class="container-fluid comm_box" style="margin:100px 15px;overflow: hidden;">
		<div class="clearfix" style="padding: 40px 0 20px;">
			<a class="today_data go_back" href="javascript:history.go(-1);">
				<img src="{{ asset('images/legendImages/goback.png') }}"/>月明细
			</a>
			<div class="sel_box">
				...
				<input id="date" name="date" value="yy-mm" type="text" class="date_pick shadow" readonly="true"/>
			</div>
		</div>
		<!--表格-->
		<div class="scroll">
...
            console.log(time);
			$('.big_bg').css({'display':'block'});
	         $.ajax({
	      		url:'/admin/home/detailDataByMonth',
	      		type:'post',
	      		async:true,
				dataType:'json',
				cache: false,
...
        });   
        ...
        var lastPage;    //最后一页
        $.ajax({
      		url:'/admin/home/detailDataByMonth',      //表格数据
      		type:'post',
      		async:true,
			dataType:'json',
			cache: false,
